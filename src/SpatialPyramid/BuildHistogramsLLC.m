...
for f = 1:length(imageFileList)
    ...
    outFName = fullfile(dataBaseDir, sprintf('%s_texton_ind_%d_LLC.mat', baseFName, params.dictionarySize));
    outFName2 = fullfile(dataBaseDir, sprintf('%s_hist_%d_LLC.mat', baseFName, params.dictionarySize));
    ...
    if ndata <= batchSize
        ...
        for j = 1:batchSize:ndata
            ...
            texton_ind.data(lo:hi,:) = ...
                LLCEncoding(features.data(lo:hi,:), dictionary, params.K);
        end
    end
    %DONT USE H
